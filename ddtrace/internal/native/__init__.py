    ...
    try:
        for entry in configurator.get_configuration():
            ...
            if source == "fleet_stable_config":
                fleet_config[entry_name] = entry["value"]
                fleet_config_ids[entry_name] = entry.get("config_id")
            elif source == "local_stable_config":
                local_config[entry_name] = entry["value"]
            else:
                print(f"Unknown configuration source: {source}, for {entry_name}")
