...
class CIVisibilityEventClient(WriterClientBase):
    def __init__(self):
        ...
        encoder.set_metadata(
            ...
            {
                "language": "python",
                "env": getenv("_CI_DD_ENV", config.env),
...
class CIVisibilityWriter(HTTPWriter):
    ...
    def __init__(
        ...
        if use_evp:
            ...
            intake_cov_url = intake_url
        if not intake_url:
            intake_url = "%s.%s" % (AGENTLESS_BASE_URL, getenv("DD_SITE", AGENTLESS_DEFAULT_SITE))
        ...
        if self._coverage_enabled:
            if not intake_cov_url:
                intake_cov_url = "%s.%s" % (AGENTLESS_COVERAGE_BASE_URL, getenv("DD_SITE", AGENTLESS_DEFAULT_SITE))
            clients.append(
        ...
        if coverage_report_upload_enabled:
            ...
            if use_evp:
                ...
                coverage_report_url = "%s.%s" % (
                    ...
                    getenv("DD_SITE", AGENTLESS_DEFAULT_SITE),
                )
