...
def _initialize_metrics(exporter_class, protocol, resource):
    ...
    try:
        ...
        if "OTEL_EXPORTER_OTLP_ENDPOINT" not in dd_environ and "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT" not in dd_environ:
            dd_environ["OTEL_EXPORTER_OTLP_METRICS_ENDPOINT"] = otel_config.exporter.METRICS_ENDPOINT
        dd_environ["OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE"] = (
            otel_config.exporter.METRICS_TEMPORALITY_PREFERENCE
        )
        dd_environ["OTEL_METRIC_EXPORT_INTERVAL"] = str(otel_config.exporter.METRICS_METRIC_READER_EXPORT_INTERVAL)
        dd_environ["OTEL_METRIC_EXPORT_TIMEOUT"] = str(otel_config.exporter.METRICS_METRIC_READER_EXPORT_TIMEOUT)
