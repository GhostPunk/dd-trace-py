...
def get_config(
    ...
    if otel_env is not None and otel_env in dd_environ:
...
def validate_otel_envs():
    user_envs = {key.upper(): value for key, value in dd_environ.items()}
    for otel_env, _ in user_envs.items():
        if (
            ...
            otel_value = dd_environ.get(otel_env, "none").lower()
            if otel_value != "none":
                _invalid_otel_config(otel_env)
...
def validate_and_report_otel_metrics_exporter_enabled():
    ...
    if "OTEL_METRICS_EXPORTER" in user_envs:
        otel_value = dd_environ.get("OTEL_METRICS_EXPORTER", "otlp").lower()
        if otel_value == "none":
