...
class LLMObs(Service):
    ...
    enabled = False
    _app_key: str = getenv("DD_APP_KEY", "")
    _project_name: str = getenv("DD_LLMOBS_PROJECT_NAME", DEFAULT_PROJECT_NAME)
    ...
    def __init__(
        ...
        self._llmobs_span_writer = LLMObsSpanWriter(
            interval=float(getenv("_DD_LLMOBS_WRITER_INTERVAL", 1.0)),
            timeout=float(getenv("_DD_LLMOBS_WRITER_TIMEOUT", 5.0)),
        ...
        self._evaluator_runner = EvaluatorRunner(
            interval=float(getenv("_DD_LLMOBS_EVALUATOR_INTERVAL", 1.0)),
            llmobs_service=self,
        )
    ...
    def enable(
        ...
        if getenv("DD_LLMOBS_ENABLED") and not asbool(getenv("DD_LLMOBS_ENABLED")):
        ...
        try:
            ...
            if config._llmobs_agentless_enabled:
                ...
                if not getenv("DD_REMOTE_CONFIGURATION_ENABLED"):
    ...
    def _patch_integrations() -> None:
        ...
        for module, _ in integrations_to_patch.items():
            ...
            if env_var in dd_environ:
                integrations_to_patch[module] = asbool(dd_environ[env_var])
        dd_patch_modules = getenv("DD_PATCH_MODULES")
