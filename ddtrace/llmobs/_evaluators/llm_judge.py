...
def _create_openai_client(client_options: Optional[dict[str, Any]] = None) -> LLMClient:
    ...
    api_key = client_options.get("api_key") or dd_environ.get("OPENAI_API_KEY")
    if not api_key:
        raise ValueError(
...
def _create_azure_openai_client(client_options: Optional[dict[str, Any]] = None) -> LLMClient:
    client_options = client_options or {}
    api_key = client_options.get("api_key") or dd_environ.get("AZURE_OPENAI_API_KEY")
    azure_endpoint = client_options.get("azure_endpoint") or dd_environ.get("AZURE_OPENAI_ENDPOINT")
    api_version = client_options.get("api_version") or dd_environ.get("AZURE_OPENAI_API_VERSION") or "2024-10-21"
    ...
    deployment_name = client_options.get("azure_deployment") or dd_environ.get("AZURE_OPENAI_DEPLOYMENT")
...
def _create_anthropic_client(client_options: Optional[dict[str, Any]] = None) -> LLMClient:
    client_options = client_options or {}
    api_key = client_options.get("api_key") or dd_environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        raise ValueError(
...
def _create_vertexai_client(client_options: Optional[dict[str, Any]] = None) -> LLMClient:
    ...
    project = (
        client_options.get("project") or dd_environ.get("GOOGLE_CLOUD_PROJECT") or dd_environ.get("GCLOUD_PROJECT")
    )
    ...
    location = (
        ...
        or dd_environ.get("GOOGLE_CLOUD_REGION")
        or dd_environ.get("GOOGLE_CLOUD_LOCATION")
        or "us-central1"
    )
...
def _create_bedrock_client(client_options: Optional[dict[str, Any]] = None) -> LLMClient:
    ...
    region_name = (
        ...
        or dd_environ.get("AWS_REGION")
        or dd_environ.get("AWS_DEFAULT_REGION")
        or "us-east-1"
    )
    ...
    profile_name = client_options.get("profile_name") or dd_environ.get("AWS_PROFILE")
    if profile_name:
        session_kwargs["profile_name"] = profile_name
    aws_access_key_id = client_options.get("aws_access_key_id") or dd_environ.get("AWS_ACCESS_KEY_ID")
    if aws_access_key_id:
        session_kwargs["aws_access_key_id"] = aws_access_key_id
    aws_secret_access_key = client_options.get("aws_secret_access_key") or dd_environ.get("AWS_SECRET_ACCESS_KEY")
    if aws_secret_access_key:
        session_kwargs["aws_secret_access_key"] = aws_secret_access_key
    aws_session_token = client_options.get("aws_session_token") or dd_environ.get("AWS_SESSION_TOKEN")
