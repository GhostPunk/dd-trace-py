...
class SessionManager:
    def __init__(self, session: TestSession) -> None:
        ...
        dd_service = dd_environ.get("DD_SERVICE")
        if dd_service:
        ...
        self.is_auto_injected = bool(getenv("DD_CIVISIBILITY_AUTO_INSTRUMENTATION_PROVIDER", ""))

        self.env = getenv("_CI_DD_ENV", getenv("DD_ENV", None))
        if self.env is None:
    ...
    def _get_test_session_name(self) -> str:
        if session_name := dd_environ.get("DD_TEST_SESSION_NAME"):
            return session_name
    ...
    def override_settings_with_env_vars(self) -> None:
        ...
        if asbool(dd_environ.get("_DD_CIVISIBILITY_ITR_PREVENT_TEST_SKIPPING", "false")):
        ...
        if asbool(dd_environ.get("_DD_CIVISIBILITY_ITR_FORCE_ENABLE_COVERAGE", "false")):
            log.debug("TIA code coverage collection is enabled by environment variable")
            self.settings.coverage_enabled = True

        if asbool(dd_environ.get("DD_CIVISIBILITY_CODE_COVERAGE_REPORT_UPLOAD_ENABLED", "false")):
