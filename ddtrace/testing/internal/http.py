...
class BackendConnectorSetup:
    ...
    def detect_setup(cls) -> BackendConnectorSetup:
        ...
        """
        if asbool(dd_environ.get("DD_CIVISIBILITY_AGENTLESS_ENABLED")):
    ...
    def _detect_agentless_setup(cls) -> BackendConnectorSetup:
        ...
        """
        site = dd_environ.get("DD_SITE") or DEFAULT_SITE
        api_key = dd_environ.get("_CI_DD_API_KEY") or dd_environ.get("DD_API_KEY")
    ...
    def _detect_evp_proxy_setup(cls) -> BackendConnectorSetup:
        ...
        """
        agent_url = dd_environ.get("_CI_DD_AGENT_URL") or dd_environ.get("DD_TRACE_AGENT_URL")
        if not agent_url:
            user_provided_host = dd_environ.get("DD_TRACE_AGENT_HOSTNAME") or dd_environ.get("DD_AGENT_HOST")
            user_provided_port = dd_environ.get("DD_TRACE_AGENT_PORT") or dd_environ.get("DD_AGENT_PORT")
...
class BackendConnectorAgentlessSetup(BackendConnectorSetup):
    ...
    def get_connector_for_subdomain(self, subdomain: Subdomain) -> BackendConnector:
        if agentless_url := dd_environ.get("DD_CIVISIBILITY_AGENTLESS_URL"):
            url = agentless_url
        else:
