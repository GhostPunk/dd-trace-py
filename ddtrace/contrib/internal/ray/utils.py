...
def _inject_context_in_env(context: Context) -> None:
    headers = {}
    _TraceContext._inject(context, headers)
    dd_environ["traceparent"] = headers.get("traceparent", "")
    dd_environ["tracestate"] = headers.get("tracestate", "")


def _extract_tracing_context_from_env() -> Optional[Context]:
    if dd_environ.get("traceparent") is not None and dd_environ.get("tracestate") is not None:
        return _TraceContext._extract(
            {
                "traceparent": dd_environ.get("traceparent"),
                "tracestate": dd_environ.get("tracestate"),
            }
        )
    return None
