...
RAY_SERVICE_NAME = dd_environ.get(RAY_JOB_NAME)
...
config._add(
    ...
    dict(
        ...
        use_entrypoint_as_service_name=asbool(getenv("DD_TRACE_RAY_USE_ENTRYPOINT_AS_SERVICE_NAME", default=False)),
        redact_entrypoint_paths=asbool(getenv("DD_TRACE_RAY_REDACT_ENTRYPOINT_PATHS", default=True)),
...
def _job_supervisor_run_wrapper(method: Callable[..., Any]) -> Any:
    async def _traced_run_method(self: Any, *args: Any, _dd_ray_trace_ctx=None, **kwargs: Any) -> Any:
        ...
        submission_id = dd_environ.get(RAY_SUBMISSION_ID)

        with long_running_ray_span(
            "actor_method.execute",
            resource=f"{self.__class__.__name__}.{method.__name__}",
            service=dd_environ.get(RAY_JOB_NAME, DEFAULT_JOB_NAME),
            span_type=SpanTypes.RAY,
            child_of=context,
            activate=True,
