...
def _is_coverage_report_upload_enabled() -> bool:
    ...
    if asbool(getenv("DD_CIVISIBILITY_CODE_COVERAGE_REPORT_UPLOAD_ENABLED", "false")):
...
def _handle_test_management(item, test_id):
    ...
    if is_quarantined and asbool(getenv("_DD_TEST_SKIP_QUARANTINED_TESTS")):
...
def _handle_coverage_patch_early(config):
    ...
    env_coverage_upload = asbool(getenv("DD_CIVISIBILITY_CODE_COVERAGE_REPORT_UPLOAD_ENABLED", "false"))
...
def pytest_configure(config: pytest_Config) -> None:
    ...
    if getenv("DD_PYTEST_USE_NEW_PLUGIN_BETA"):
